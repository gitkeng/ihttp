...
import (
	...
	"github.com/gitkeng/ihttp/util/structutil"
)
...
func TestCopyFromStructToSlice(t *testing.T) {
	...
	if err := structutil.Copy(employees, &user); err != nil && len(employees) != 0 {
		t.Errorf("Copy to unaddressable value should get error")
	}

	if structutil.Copy(&employees, &user); len(employees) != 1 {
	...
	employees2 := &[]Employee{}
	if structutil.Copy(&employees2, user); len(*employees2) != 1 {
		t.Errorf("Should only have one elem when copy struct to slice")
	} else {
	...
	employees3 := []*Employee{}
	if structutil.Copy(&employees3, user); len(employees3) != 1 {
		t.Errorf("Should only have one elem when copy struct to slice")
	} else {
	...
	employees4 := &[]*Employee{}
	if structutil.Copy(&employees4, user); len(*employees4) != 1 {
		t.Errorf("Should only have one elem when copy struct to slice")
	} else {
...
func TestCopyFromSliceToSlice(t *testing.T) {
	...
	if structutil.Copy(&employees, users); len(employees) != 2 {
	...
	employees2 := &[]Employee{}
	if structutil.Copy(&employees2, &users); len(*employees2) != 2 {
		t.Errorf("Should have two elems when copy slice to slice")
	} else {
	...
	employees3 := []*Employee{}
	if structutil.Copy(&employees3, users); len(employees3) != 2 {
		t.Errorf("Should have two elems when copy slice to slice")
	} else {
	...
	employees4 := &[]*Employee{}
	if structutil.Copy(&employees4, users); len(*employees4) != 2 {
		t.Errorf("Should have two elems when copy slice to slice")
	} else {

...
func TestCheckMobile(t *testing.T) {
	iso := phoneutil.GetISO3166ByCountry("th")
	t.Logf("iso: %s", iso)
	iso3166s := phoneutil.GetISO3166()
	for _, iso3166 := range iso3166s {
		t.Logf("conuntry code: %s, country name: %s", iso3166.Alpha2, iso3166.CountryName)
	}
}

func TestFormatMobile(t *testing.T) {
	for _, tt := range mobFormatTests {
		number := phoneutil.Parse(tt.input, tt.country)
...
func TestFormatWithLandLine(t *testing.T) {
	for _, tt := range mobWithLLFormatTests {
		number := phoneutil.ParseWithLandLine(tt.input, tt.country)
		if number != tt.expected {
			t.Errorf("Parse(number=`%s`, country=`%s`): expected `%s`, actual `%s`", tt.input, tt.country, tt.expected, number)
		}
	}
...
func TestIndiaMobileNumber(t *testing.T) {
	for _, tt := range indiaMobileTests {
		country := phoneutil.GetISO3166ByNumber(tt.input, false)

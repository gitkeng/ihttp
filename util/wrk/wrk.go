		...
		for j := 0; j < pipeliningFactor; j++ {
			go func() {
				...
				for {
					...
					if curTemplate.MaxIdleConnDuration < defaultMaxIdleConnDuration {
						c.MaxIdleConnDuration = defaultMaxIdleConnDuration
					} else {
						c.MaxIdleConnDuration = curTemplate.MaxIdleConnDuration
					}
...
type LoadTestTemplate struct {
	...
	// MaxIdleConnDuration is the maximum amount of time an idle (keep-alive) connection will remain idle before closing default 10 seconds
